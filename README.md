...
2. threads.js library example, https://www.npmjs.com/package/threads and it wrapps web worker and worke thread. It works perfectly. Only one trickt is to add `outputCapture` in VSCode's launch.json to show the logs in worker threads. One limitation is that VSCode breakpoints will not stop at the code in worker threads.
